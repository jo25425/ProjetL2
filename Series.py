...
Idées
* Ajouter des groupes à l'algo k-means jusqu'à ce que tous les groupes soient
  de taille inférieure à n(Mylil_matrix.subgroups!)
* Représenter les séries dans le plan où la droite des abscisses est le cosinus
  similarité avec un vecteur et la droite des ordonnées, celui avec un autre
  vecteur
* Représenter les series dans un espace de dimension n comme celui d'au dessus,
  on peut alors trier les vecteurs qu'on représente(totalité des séries) par
  norme pour établir une liste triée qu'une personne pourrait aimer
'''
...
class Projet():
    def __init__(self, Dumps=pathDumps, Data=pathData):
        ...
        self.tags = {
            'WP$', 'RB', 'JJR', 'CC', 'VVZ', 'IN', 'PP$', 'VVG', 'DT', 'LS',
            'VBD', 'VBZ', 'VBP', 'RBR', 'MD', 'NP', 'RP', 'SENT', "''", 'WP',
            'WDT', 'VVN', 'PP', 'WRB', 'JJ', 'NPS', 'PDT', 'VB', 'FW', 'CD',
            'VVD', '$', 'UH', 'VV', 'SYM', 'VBG', 'VBN', '``', 'VHP', 'TO',
            'IN/that', ':', 'NNS', 'EX', 'VHD', 'NN', 'VH', 'VHG', '#',
            'VHZ', ',', 'JJS', '(', ')', 'VVP', 'POS', 'RBS', 'VHN'
        }
    ...
    def TxtTrt(self, Text):
        '''
        Prend en argument une chaine de charactères, retourne une liste de mots
        '''
    ...
    def _MergeDelTags(self, TagDict, data=None, WrdDict=None):
        '''
        TagDict doit être un dictionnaire dont les clés sont des tuples de
        tags, et les valeurs sont les nouveaux noms à donner à ces
        rassemblements de tags.
    ...
    def FlagTags(self, TagGrps, WrdDict=None):
        ...
        FlagTags([('DT','NP'),('NN','VB')]) will return a first lis of every
        word starting with DT or NP then a second list of every word starting
        with NN or VB.
    ...
    def CleanUpStatsMat(self, maxDF=100, minDF=5, Smax=5000):
        ...
        Currently removes columns with a Document Frequency DF higher than
        maxDF % or lower than minDF.(flat amount)
                :param maxDF:
                :param minDF:
        '''
        RowToDel = []
    ...
    def dump_old(self, name=None, EpiMat=True, WrdKey=True, SsnKey=True, StatsMat=True):
        '''
        Ecrit self.EpiMat, self.WrdKey, self.SsnKey, self.StatsMat sur le
        disque sous forme de fichiers .dump
    ...
    def load_old(self, name='100', EpiMat=True, WrdKey=True, SsnKey=True, StatsMat=True):
        '''
        Charge self.EpiMat, self.WrdKey, self.SsnKey, self.StatsMat depuis le
        répertoire spécifié par self.pathf.
        Les fichiers EpiMat.dump et StatsMat.dump sont au format renvoyé par
        scipy.io.mmwrite tandis que self.WrdKey et self.SsnKey sont au format
        utilisé par le protocole par défaut de pickle.
        '''
        if EpiMat:
    ...
    def AddSerie_old(self, Path):
        '''Path doit lier à un dossier dont le nom est de la forme spécifiée
        par SsnPat ('(\d+)___(\S+)'). Le séparateur du path doit être '/'
        :type Path: String
        '''
        ...
        for Season in LstSsn:
            ...
            for Epi in LstEpi:
                ...
                try:  # test de l'encoding utf8
                    ...
                    # En cas d'erreur, essai de l'encoding latin-1
                    try:
                        File = open(Path + '/' + Season + '/' + Epi, 'r', encoding="latin-1")
                        Contents = File.read()

                    # Si toujours erreur, on passe au suivant en ajoutant cela aux erreurs
                    except UnicodeDecodeError:
                        File.close()
